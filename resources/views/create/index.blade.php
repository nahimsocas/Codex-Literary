                ...
                @else
                    @foreach ($projects as $value)            
                        <article class="project--miniview">
                            <a href="{{ route('create.archive', [$value->url, hash('crc32', $value->CLnumber, false)]) }}" target="_self">
                                <h1>{{ $value->title }}</h1>
                                <h2>Author: {{ $value->author }}</h2>
                                <p>Synopsis: {{ $value->description }}</p>
                            </a>
                            <section class="project--edit">
                                <a href="{{ route('create.edit', [$value->url, hash('crc32', $value->CLnumber, false)]) }}"><small>Edit</small></a>
                                <form action="{{ route('create.destroy', [$value->url, hash('crc32', $value->CLnumber, false)]) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit"><small>Delete</small></button>
                                </form>
                            </section>
                        </article>                    
                    @endforeach
